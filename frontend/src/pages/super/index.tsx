...
export default function SuperOverviewPage() {
  ...
  const schoolCount = useMemo(() => {
    return dashboardData?.organizations?.length || 0;
  }, [dashboardData]);

  const totalStudents = useMemo(() => {
    return dashboardData?.stats?.totalStudents || 0;
  }, [dashboardData]);

  const mrr = useMemo(() => {
    return dashboardData?.stats?.mrr || 0;
  }, [dashboardData]);
  ...
  const completionRate = useMemo(() => {
    return dashboardData?.stats?.assessmentCompletion || 0;
  }, [dashboardData]);

  const healthMetrics = useMemo(() => {
    return {
      dbHealth: healthData?.databaseHealth || dashboardData?.health?.databaseHealth || "100% OPERATIONAL",
      latency: healthData?.apiLatencyMs || dashboardData?.health?.apiLatencyMs || 15,
      cacheHits: healthData?.pwaCacheHitRate || dashboardData?.health?.pwaCacheHitRate || 98.5,
    };
