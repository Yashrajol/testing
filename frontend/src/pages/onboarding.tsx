...
export default function OnboardingPage() {
  ...
  const handleFinishOnboarding = async () => {
    ...
    try {
      if (user?.id) {
        await apiClient.post("/api/v1/auth/onboarding", {
          userId: user.id,
          role: selectedRole,
      ...
      if (selectedRole === "PARENT") {
        toast.success("Parent profile setup complete!", {
          description: childMobileOrEmail 
            ? `Linked with student account (${childMobileOrEmail}). Redirecting to Parent Dashboard...` 
      ...
      updateUser({ role: selectedRole });
      if (selectedRole === "PARENT") {
        toast.success("Parent profile setup complete! Redirecting to Parent Dashboard...");
        navigate({ to: "/dashboard/parent" });
      } else {
        toast.success("Profile setup done! Starting your free diagnostic assessment...");
        navigate({ to: "/assessments" });
