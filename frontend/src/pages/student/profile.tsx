...
export default function ProfileManagementPage() {
  ...
  // Initialize profile state from user auth context or localStorage
  const [profile, setProfile] = useState(() => {
    ...
    return {
      ...
      email: user?.email || "student@vedhkrit.edu",
      ...
      emergencyPhone: "",
      schoolName: user?.schoolName || "Vedhkrit Partner School",
      board: "CBSE",
      className: user?.grade ? `Grade ${user.grade}` : "Grade 10",
      section: "A",
      rollNumber: "",
      admissionNumber: "",
      studentId: user?.id ? `VED-${user.id.slice(0, 8).toUpperCase()}` : "VED-2026-0923",
      ...
      },
      {
        onSuccess: () => {
          ...
          toast.success("Profile details updated successfully!", {
            description: "Your changes have been saved to your student database record.",
          ...
          <GlassCard className="p-6 text-center flex flex-col items-center bg-white border border-slate-100 shadow-xs">
            ...
            <div className="w-full text-left mt-6">
              ...
              <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                <div className="bg-brand-blue h-1.5 rounded-full transition-all duration-500" style={{ width: `${completionPercentage}%` }}></div>
              </div>
              <span className="text-[8.5px] text-text-muted mt-1.5 block font-semibold leading-normal">
                {completionPercentage === 100 ? "🎉 Profile 100% complete!" : "Complete all categories to reach 100% profile strength."}
              </span>
            </div>
          </GlassCard>
