...
function AcademicsPage() {
  ...
  const semesterProgress = useMemo(() => {
    ...
    const total = subjects.reduce((sum, s) => sum + (s.progress?.syllabusCoverage ?? 0), 0);
      ...
      <AnimatePresence mode="wait">
        {!selectedSubjectId ? (
          ...
          >
            ...
            {subjects && subjects.length > 0 && subjects[0].enrollment && (
              <motion.div variants={itemVariants}>
                <div className="p-5 rounded-2xl border border-slate-100 bg-white grid grid-cols-2 sm:grid-cols-5 gap-4 text-[11px] font-bold text-text-muted">
                  ...
                  <div><span className="block text-[8px] uppercase tracking-wider text-slate-400 font-bold mb-0.5">Class & Section</span>{subjects[0].enrollment.classSection || "Class IX - A"}</div>
                  <div><span className="block text-[8px] uppercase tracking-wider text-slate-400 font-bold mb-0.5">Batch</span>{subjects[0].enrollment.batch || "Foundation A1"}</div>
                  <div><span className="block text-[8px] uppercase tracking-wider text-slate-400 font-bold mb-0.5">Academic Year</span>{subjects[0].enrollment.academicYear || "2026 - 27"}</div>
                </div>
              </motion.div>
            )}
