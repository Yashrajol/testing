...
import { Attempt, AssessmentResult } from "@/features/assessments/types";
...
function AssessmentsPage() {
  ...
  const practice = useMemo(() => assessments?.filter(a => a.status === "practice") ?? [], [assessments]);
              ...
              <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-5">
                <div>
                  <h3 className="font-display text-lg font-bold text-text-heading">
                    {upcoming.find(t => t.id === activeQuiz)?.chapter || practice.find(t => t.id === activeQuiz)?.chapter || "Assessment"}
                  </h3>
